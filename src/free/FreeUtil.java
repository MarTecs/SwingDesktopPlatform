...
public class FreeUtil {
                ...
                for (int i = 0; i < modules.getLength(); i++) {
                    ...
                    if (moduleNode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
                        if (moduleNode.getNodeName().equalsIgnoreCase("module")) {
                            ...
                            FreeOutlookBar bar = outlookPane.addBar(text, icon, iconSelected);
    ...
    private static void readNodeShortcuts(org.w3c.dom.Node moduleNode, TDataBox box, ActionListener shortcutAction) {
                            ...
                            for (int j = 0; j < shortcuts.getLength(); j++) {
                                ...
                                if (shortcut.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
                                    if (shortcut.getNodeName().equalsIgnoreCase("shortcut")) {
                                        String text = getStringAttribute(shortcut, "text");
                                        String tooltip = getStringAttribute(shortcut, "tooltip");
                                        String iconURL = getStringAttribute(shortcut, "icon");
                                        String command = getStringAttribute(shortcut, "action");
    ...
    private static void readNodeButtons(org.w3c.dom.Node moduleNode, FreeNode twaverNode, ActionListener action) {
        NodeList buttons = moduleNode.getChildNodes();
        if (buttons != null) {
            for (int i = 0; i < buttons.getLength(); i++) {
                org.w3c.dom.Node buttonNode = buttons.item(i);
                if (buttonNode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
                    boolean isButton1 = buttonNode.getNodeName().equalsIgnoreCase("button1");
                    boolean isButton2 = buttonNode.getNodeName().equalsIgnoreCase("button2");
                    boolean isButton3 = buttonNode.getNodeName().equalsIgnoreCase("button3");
